...
Public Sub SetStatus(ByVal Status As String)
   ...
   On Error GoTo errorhandler
    ...
    If frmLoad.Visible Then Exit Sub
    Call TextAdd(Status)
    DoEvents
...
Sub SetLoadingProgress(Caption As String, Stage As Long, StageProg As Double)
   ...
   On Error GoTo SetLoadingProgress_Error
    ...
    frmLoad.lblProg.Caption = Caption & " " & CStr(Int(StageProg * 100)) & "% done. - " & CStr(Int(prog)) & "% overall."
    frmLoad.pbarLoading.Value = prog
