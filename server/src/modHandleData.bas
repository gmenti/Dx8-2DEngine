...
Private Sub HandleNewAccount(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    Dim CaptchaGerado As String
    Dim CaptchaDigitado As String
    Dim Email As String

    On Error GoTo errorhandler

    If IsPlaying(Index) Or IsLoggedIn(Index) Then Exit Sub
    
    Set Buffer = New clsBuffer
    Buffer.WriteBytes data()
    Usuario = Trim$(Buffer.ReadString)
    Email = Trim$(Buffer.ReadString)
    Senha = Trim$(Buffer.ReadString)
    Senha2 = Trim$(Buffer.ReadString)
    CaptchaGerado = Trim$(Buffer.ReadString)
    CaptchaDigitado = Trim$(Buffer.ReadString)
    
    If Not validarNovaConta(Usuario, Email, Senha, Senha2, CaptchaGerado, CaptchaDigitado) Then
        Call AlertMsg(Index, MENU_ERRO_MSG)
        Exit Sub
    End If
    
    Call AddAccount(Index, Usuario, Senha, Email)
    Call TextAdd(Usuario & " é um novo usuário.")
    Call AddLog(Usuario & " é um novo usuário.", PLAYER_LOG)
    
    ' Load the player
    Call LoadPlayer(Index, Usuario)
    Call SendCharactersToPlayer(Index)
            
    ' Show the player up on the socket status
    Call AddLog(GetPlayerLogin(Index) & " conectou-se. (" & GetPlayerIP(Index) & ")", PLAYER_LOG)
    Call TextAdd(GetPlayerLogin(Index) & " conectou-se. (" & GetPlayerIP(Index) & ")")
...
Private Sub HandleLogin(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    If cMajor <> CLng(App.Major) Or cMinor <> CLng(App.Minor) Or cRevision <> CLng(App.Revision) Then
        Call AlertMsg(Index, "Versão desatualizada, reinicie o jogo e tente novamente!")
        Exit Sub
    End If
    
    If Not validarLogin(Usuario, Senha) Then
        Call AlertMsg(Index, MENU_ERRO_MSG)
        Exit Sub
    End If
...
Public Sub HandleCharSlot(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    'Usar char
    If Len(Trim$(Player(Index).characters(slot).Name)) > 0 Then
...
Private Sub HandleAddChar(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    If CharExist(Index, slot) Then
        Call AlertMsg(Index, "Já existe um personagem no slot " & slot)
        Exit Sub
    End If

    Call AddChar(Index, Nome, Sexo, Classe, Sprite, slot)
    Call AddLog("Personagem " & Nome & " foi adicionado ao usuario " & GetPlayerLogin(Index), PLAYER_LOG)
    HandleUseChar Index, slot
...
Dim Buffer As clsBuffer
   ...
   On Error GoTo errorhandler
    ...
    Set Buffer = New clsBuffer
    Buffer.WriteBytes data()
    If ServerOnline = False Then frmLoad.Show
    'frmLoad.fraConnecting.Visible = False
    ServerOnline = False
