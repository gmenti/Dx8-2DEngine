...
Dim buffer As clsBuffer
   ...
   On Error GoTo errorhandler
    ...
    Set buffer = Nothing
    menuPreto = False
    Call SetStatus("Iniciando o jogo...")
...
Dim buffer As clsBuffer, i As Long, numchars As Long, CharName As String, Level As String, Class As String, X As Long
   ...
   On Error GoTo errorhandler
    ...
    'frmMain.Visible = True
    ' save options
    'Servers(ServerIndex).Username = Trim$(txtLUsuario)

    'If Servers(ServerIndex).SavePass = 0 Then
    '    Servers(ServerIndex).Password = vbNullString
    'Else
    '    Servers(ServerIndex).Password = Trim$(txtLSenha)
   ' End If
    
    SaveServers
    Call limparCamposLogin
    
    MenuStage = 4
    For i = 1 To numchars
        CharName = Trim$(buffer.ReadString)
        Level = Trim$(buffer.ReadString)
        Class = Trim$(buffer.ReadString)
        
        CharSelection(i).Name = CharName
        CharSelection(i).Level = "Level " & Level
        CharSelection(i).Class = Class
        CharSelection(i).Sex = buffer.ReadLong
        
        For X = 1 To Equipment.Equipment_Count - 1
            CharSelection(i).Paperdoll(X) = buffer.ReadLong
        Next
        ...
        If Trim$(CharSelection(i).Name) = "" Then
            CharSelection(i).Name = "Livre"
            CharSelection(i).Level = ""
        End If
