...
Public Function InitDX8() As Boolean
   ...
   On Error GoTo errorhandler
    ...
    Direct3D_Window.BackBufferWidth = BBWidth ' frmMain.picScreen.ScaleWidth 'Match the backbuffer width with the display width
    Direct3D_Window.BackBufferHeight = BBHeight 'frmMain.picScreen.ScaleHeight 'Match the backbuffer height with the display height
...
Dim i As Long
   ...
   On Error GoTo errorhandler
       ...
       Select Case Options.Render
        Case 1 ' hardware
            If LoadDirectX(D3DCREATE_HARDWARE_VERTEXPROCESSING, frmMain.picScreen.hwnd) <> 0 Then
                Options.Render = 0
                SaveOptions
            ...
            End If
        Case 2 ' mixed
            If LoadDirectX(D3DCREATE_MIXED_VERTEXPROCESSING, frmMain.picScreen.hwnd) <> 0 Then
                Options.Render = 0
                SaveOptions
            ...
            End If
        Case 3 ' software
            If LoadDirectX(D3DCREATE_SOFTWARE_VERTEXPROCESSING, frmMain.picScreen.hwnd) <> 0 Then
                Options.Render = 0
                SaveOptions
...
Dim X2 As Long, Y2 As Long
   ...
   On Error GoTo errorhandler
                        ...
                        Select Case i
                            ...
                            Case SpriteEnum.Pants
                                If GetPlayerEquipment(Index, Equipment.Armor) > 0 Then
                                    If Item(GetPlayerEquipment(Index, Equipment.Armor)).Paperdoll = 0 Then
    ...
    RenderTexture Tex_CharHair(Player(Index).Sprite(SpriteEnum.Hair)), ConvertMapX(X2), ConvertMapY(Y2), rec2.Left, rec2.Top, rec2.Right, rec2.Bottom, rec2.Right, rec2.Bottom, -1, True
   ...
   On Error GoTo errorhandler
    ...
    dRect = NCPreviewBounds
        ...
        'If NewCharHead > 0 And NewCharHead <= NumFaceHeads Then
            If Tex_CharBodies(NewCharHead).Texture > 0 Then
                RenderTexture Tex_CharBodies(NewCharHead), dRect.Left, dRect.Top, 0, 0, Tex_CharBodies(NewCharHead).Width / 4, Tex_CharBodies(NewCharHead).Height / 4, Tex_CharBodies(NewCharHead).Width / 4, Tex_CharBodies(NewCharHead).Height / 4
            End If
        'End If
        
        'If NewCharShirt > 0 And NewCharShirt <= NumFaceShirts Then
            If Tex_CharShirts(NewCharShirt).Texture > 0 Then
               RenderTexture Tex_CharShirts(NewCharShirt), dRect.Left, dRect.Top, 0, 0, Tex_CharShirts(NewCharShirt).Width / 4, Tex_CharShirts(NewCharShirt).Height / 4, Tex_CharShirts(NewCharShirt).Width / 4, Tex_CharShirts(NewCharShirt).Height / 4
            End If
        'End If
        
        'If NewCharHair > 0 And NewCharHair <= NumFaceHair Then
            If Tex_CharHair(NewCharHair).Texture > 0 Then
                RenderTexture Tex_CharHair(NewCharHair), dRect.Left, dRect.Top, 0, 0, Tex_CharHair(NewCharHair).Width / 4, Tex_CharHair(NewCharHair).Height / 4, Tex_CharHair(NewCharHair).Width / 4, Tex_CharHair(NewCharHair).Height / 4
            End If
        'End If
        
        If NewCharSex = SEX_MALE Then
            RenderTexture Tex_MaleLegs(1), dRect.Left, dRect.Top, 0, 0, Tex_MaleLegs(1).Width / 4, Tex_MaleLegs(1).Height / 4, Tex_MaleLegs(1).Width / 4, Tex_MaleLegs(1).Height / 4
        Else
            RenderTexture Tex_FemaleLegs(1), dRect.Left, dRect.Top, 0, 0, Tex_FemaleLegs(1).Width / 4, Tex_FemaleLegs(1).Height / 4, Tex_FemaleLegs(1).Width / 4, Tex_FemaleLegs(1).Height / 4
        End If
...
Dim Width As Long, Height As Long, TextureNum As DX8TextureRec, i As Long
Dim x As Long, y As Long
Dim PaperdollNum As Long

   On Error GoTo errorhandler

    dRect = CharPreviewBounds
    If SelectedChar <= 0 Or SelectedChar > UBound(CharSelection) Then Exit Sub
    If CharSelection(SelectedChar).Level = "" Then Exit Sub

    If CharSelection(SelectedChar).Face(FaceEnum.Head) > 0 And CharSelection(SelectedChar).Face(FaceEnum.Head) <= NumFaceHeads Then
        If Tex_CharBodies(CharSelection(SelectedChar).Face(FaceEnum.Head)).Texture > 0 Then
            RenderTexture Tex_CharBodies(CharSelection(SelectedChar).Face(FaceEnum.Head)), dRect.Left, dRect.Top, 0, 0, Tex_CharBodies(CharSelection(SelectedChar).Face(FaceEnum.Head)).Width / 4, Tex_CharBodies(CharSelection(SelectedChar).Face(FaceEnum.Head)).Height / 4, Tex_CharBodies(CharSelection(SelectedChar).Face(FaceEnum.Head)).Width / 4, Tex_CharBodies(CharSelection(SelectedChar).Face(FaceEnum.Head)).Height / 4
        End If
    End If
    ...
    If CharSelection(SelectedChar).Face(FaceEnum.Shirt) > 0 And CharSelection(SelectedChar).Face(FaceEnum.Shirt) <= NumFaceShirts Then
        If Tex_CharShirts(CharSelection(SelectedChar).Face(FaceEnum.Shirt)).Texture > 0 Then
           RenderTexture Tex_CharShirts(CharSelection(SelectedChar).Face(FaceEnum.Shirt)), dRect.Left, dRect.Top, 0, 0, Tex_CharShirts(CharSelection(SelectedChar).Face(FaceEnum.Shirt)).Width / 4, Tex_CharShirts(CharSelection(SelectedChar).Face(FaceEnum.Shirt)).Height / 4, Tex_CharShirts(CharSelection(SelectedChar).Face(FaceEnum.Shirt)).Width / 4, Tex_CharShirts(CharSelection(SelectedChar).Face(FaceEnum.Shirt)).Height / 4
        End If
    End If
    
    If CharSelection(SelectedChar).Face(FaceEnum.Hair) > 0 And CharSelection(SelectedChar).Face(FaceEnum.Hair) <= NumFaceHair Then
        If Tex_CharHair(CharSelection(SelectedChar).Face(FaceEnum.Hair)).Texture > 0 Then
            RenderTexture Tex_CharHair(CharSelection(SelectedChar).Face(FaceEnum.Hair)), dRect.Left, dRect.Top, 0, 0, Tex_CharHair(CharSelection(SelectedChar).Face(FaceEnum.Hair)).Width / 4, Tex_CharHair(CharSelection(SelectedChar).Face(FaceEnum.Hair)).Height / 4, Tex_CharHair(CharSelection(SelectedChar).Face(FaceEnum.Hair)).Width / 4, Tex_CharHair(CharSelection(SelectedChar).Face(FaceEnum.Hair)).Height / 4
        End If
    End If
    
    If CharSelection(SelectedChar).Paperdoll(Equipment.Armor) = 0 Then
        If CharSelection(SelectedChar).Sex = SEX_MALE Then
            RenderTexture Tex_MaleLegs(1), dRect.Left, dRect.Top, 0, 0, Tex_MaleLegs(1).Width / 4, Tex_MaleLegs(1).Height / 4, Tex_MaleLegs(1).Width / 4, Tex_MaleLegs(1).Height / 4
        Else
            RenderTexture Tex_FemaleLegs(1), dRect.Left, dRect.Top, 0, 0, Tex_FemaleLegs(1).Width / 4, Tex_FemaleLegs(1).Height / 4, Tex_FemaleLegs(1).Width / 4, Tex_FemaleLegs(1).Height / 4
        End If
    ...
    ' check for paperdolling
    For i = 1 To UBound(PaperdollOrder)
        PaperdollNum = CharSelection(SelectedChar).Paperdoll(PaperdollOrder(i))
        If PaperdollNum > 0 Then
            If PaperdollNum <= NumPaperdolls Then
                RenderTexture Tex_Paperdoll(PaperdollNum), dRect.Left, dRect.Top, 0, 0, Tex_Paperdoll(PaperdollNum).Width / 4, Tex_Paperdoll(PaperdollNum).Height / 4, Tex_Paperdoll(PaperdollNum).Width / 4, Tex_Paperdoll(PaperdollNum).Height / 4
            End If
        End If
    Next
...
Dim rec_pos As rect, srcRect As D3DRECT, destRect As D3DRECT
   ...
   Direct3D_Device.Clear 0, ByVal 0, D3DCLEAR_TARGET, D3DColorARGB(0, 0, 0, 0), 1#, 0
            ...
            Next
                ...
                Next
            Next
                    If DrawThunder > 0 Then RenderTexture Tex_White, 0, 0, 0, 0, frmMain.picScreen.ScaleWidth, frmMain.picScreen.ScaleHeight, 32, 32, D3DColorRGBA(255, 255, 255, 160), True, True: DrawThunder = DrawThunder - 1
                    DrawWalkArrow
                    ' Get rec
            With rec
                .Top = Camera.Top
            ...
            If InMapEditor Then Call DrawMapAttributes
                    If FadeAmount > 0 Then RenderTexture Tex_Fade, 0, 0, 0, 0, frmMain.picScreen.ScaleWidth, frmMain.picScreen.ScaleHeight, 32, 32, D3DColorRGBA(255, 255, 255, FadeAmount)
            If FlashTimer > GetTickCount Then RenderTexture Tex_White, 0, 0, 0, 0, frmMain.picScreen.ScaleWidth, frmMain.picScreen.ScaleHeight, 32, 32, -1, True, True
